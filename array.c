...
static void     ArrayIterator_setval(ArrayIteratorClass *this, ...)
{
    ...
    type = this->_array->_type;
    va_start(ap, this);
    value = va_arg(ap, void *);
    delete(this->_array);
    this->_array = new(type, value);
    va_end(ap);
}
...
static void     Array_ctor(ArrayClass *this, va_list *args)
{
    ...
    list = malloc(sizeof(class->__size__) * size);
    for (size_t i = 0; i < size; i++) {
        va_copy(cpy, *args);
        list[i] = va_new(class, &cpy);
        va_end(cpy);
    }
    this->_tab = list;
    this->_size = size;
...
static void     Array_setitem(ArrayClass *this, ...)
{
    ...
    this->_tab[index] = va_new(class, &ap);
    va_end(ap);
}
